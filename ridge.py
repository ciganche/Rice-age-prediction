...
#             cv2_train                  cv2_test
    ...
    dummie_estimator = linear_model.Ridge()
    ...
    for param_selection_dictionary in searcher.cv_results_["params"]:
        ...
        for (train_indices_CV1, test_indices_CV1) in StratifiedKFold(n_splits=5, shuffle=True, random_state=7).split(X, cv_categories):
            ...
            selector_estimator = linear_model.Ridge(**param_selection_dictionary) #unpacks the dictionary and passes the values
...
initial_param_dictionary = {
                            "alpha": [17,18,18.5,19,20,21,22,23,24,25,26,27,29,30],
                            "solver": ["svd", "cholesky", "lsqr", "sparse_cg","auto"],
