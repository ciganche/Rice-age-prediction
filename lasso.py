...
#             cv2_train                  cv2_test
    ...
    dummie_estimator = linear_model.Lasso()
    ...
    for param_selection_dictionary in searcher.cv_results_["params"]:
        ...
        for (train_indices_CV1, test_indices_CV1) in StratifiedKFold(n_splits=5, shuffle=True, random_state=7).split(X, cv_categories):
            ...
            selector_estimator = linear_model.Lasso(**param_selection_dictionary) #unpacks the dictionary and passes the values
...
initial_param_dictionary = {
                            "alpha": [0.01,0.1,0.1,0.5,0.75,1,1.25,1.5,1.75,2,3,3.5],
                            "max_iter": [5000]
                            }
