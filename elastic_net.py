...
def remove_low_otus(train_joined, test_joined):
    ...
    condition_dataframe = (temp.sum() <= 20)
    columns_to_remove = []
...
#             cv2_train                  cv2_test
    ...
    dummie_estimator = linear_model.ElasticNet()
    ...
    for param_selection_dictionary in searcher.cv_results_["params"]:
        ...
        for (train_indices_CV1, test_indices_CV1) in StratifiedKFold(n_splits=5, shuffle=True, random_state=7).split(X, cv_categories):
            ...
            selector_estimator = linear_model.ElasticNet(**param_selection_dictionary) #unpacks the dictionary and passes the values
...
def write_exact_feature_importance(selector, df, file_name):
    dataframe = df.iloc[:, 2:]
    all_features = list(dataframe.columns.values)
    all_features = np.array(all_features)
    feature_indices = np.asarray(np.where(selector.support_ == True))
    feature_indices = feature_indices[0]
    microbe_names = all_features[feature_indices]

    (X, Y, stats) = extract_ml_format_data(df)
    X_new = X[:,feature_indices]
    estimator = selector.estimator
    estimator.fit(X_new,Y)
    original_coefs = estimator.coef_

    for i in range(0,len(microbe_names)):
        microbe_names[i] = microbe_names[i] + "," + str(original_coefs[i])

    important_features_dict = {}
    for index, x in enumerate(abs(estimator.coef_)):
        important_features_dict[microbe_names[index]] = x

    important_features_list = sorted(important_features_dict, key=important_features_dict.get, reverse=True)

    f = open(file_name, "w")
    content = ""
    for microbe in important_features_list:
        content = content + microbe + "," + str(important_features_dict[microbe]) + "\n"
    f.write(content)
    f.close()
...
level_file = "familia.tsv"
method_location = "elastic_net/"
...
write_exact_feature_importance(selector, train_joined, "feature_importance.txt")
